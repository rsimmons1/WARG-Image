...
public class recognizer extends PApplet {
...
PImage img;
int red = color(255, 0, 42);
int blue = color(31, 123, 255);
...
public void setup() {
  for (int q = 0; q < list.length; q++) {
    ...
    img.resize(600, 600);
    wid = PApplet.parseInt(img.width/ 10)*10;
    hig = PApplet.parseInt(img.height/ 10)*10;
...
public Boolean [][] submatrix(Boolean[][] matrix, int posx, int posy, int dimx, int dimy) {
...
public void printer(Boolean[][] matrix) {
...
public boolean inColor(int given, int col, float error, float bri, float sat) {
...
public boolean isWhite(int given) {
...
public Box buildShape(Boolean [][] Matrix, Box B, int count) {
  Boolean up = false;
  Boolean down = false;
  Boolean side = false;
  Boolean Lside = false;
...
public int BooleanCount(Boolean [][] Matrix) {
  int count = 0;
  for (int y = 0; y < Matrix[0].length; y++) {
    for (int x = 0; x < Matrix.length; x++) {
      if (Matrix[x][y]) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "recognizer" };
    if (passedArgs != null) {
